      ...
      {editProduct&&(
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.3)",zIndex:500,display:"flex",alignItems:"center",justifyContent:"center",padding:20}} onClick={e=>e.target===e.currentTarget&&setEditProduct(null)}>
          <div style={{background:theme.surface,border:`1px solid ${theme.border}`,borderRadius:10,padding:28,width:"100%",maxWidth:560,maxHeight:"90vh",overflowY:"auto",boxShadow:"0 8px 32px rgba(0,0,0,0.12)"}}>
            ...
            <div style={{marginBottom:12}}><label style={{fontSize:11,fontWeight:600,color:theme.muted,textTransform:"uppercase",display:"block",marginBottom:5}}>Product ID</label><input style={{...inp,background:"transparent",cursor:"default"}} value={editProduct.product_id||""} readOnly /></div><div style={{marginBottom:12}}><label style={{fontSize:11,fontWeight:600,color:theme.muted,textTransform:"uppercase",display:"block",marginBottom:5}}>Nombre</label><input style={inp} value={editProduct.name} onChange={e=>setEditProduct({...editProduct,name:e.target.value})}/></div>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:12}}>
...
setData({ byRevenue: allRev.slice(0, 15), byUnits: allUnits.slice(0, 15), byCustomers: allCust.slice(0, 15), allByRevenue: allRev, allByUnits: allUnits, allByCustomers: allCust, priorityA, highSignal: ranked.filter(r => r.highSignal), repeat: ranked.filter(r => r.repeat), totalProducts: ranked.length });
      setLoading(false);
    }
    load();
  ...
  const colCfg = { revenue: { mainLabel: 'Revenue', mainKey: 'revenue', mainFmt: fmtM, secLabel: 'Unidades', secKey: 'units', secFmt: (n) => n }, units: { mainLabel: 'Unidades', mainKey: 'units', mainFmt: (n) => n, secLabel: 'Revenue', secKey: 'revenue', secFmt: fmtM }, customers: { mainLabel: 'Clientes', mainKey: 'customers', mainFmt: (n) => n, secLabel: 'Revenue', secKey: 'revenue', secFmt: fmtM } };
  const tableRows = tab === 'revenue' ? (data ? data.byRevenue : []) : tab === 'units' ? (data ? data.byUnits : []) : (data ? data.byCustomers : []); const cfg = colCfg[tab] || colCfg.revenue;
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 20, flexWrap: 'wrap', gap: 12 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}><DateRangeSelector preset={preset} setPreset={setPreset} customFrom={customFrom} setCustomFrom={setCustomFrom} customTo={customTo} setCustomTo={setCustomTo} /><div onClick={() => { if (!data) return; const rows = tab === 'signals' ? data.priorityA : (tab === 'revenue' ? data.allByRevenue : tab === 'units' ? data.allByUnits : data.allByCustomers); const headers = ['Rank','Product Name','Product ID','URL','Revenue','Units','Customers','Orders','High Signal','Repeat','Priority A']; const dSign = String.fromCharCode(36); const lines = [headers.join(',')]; rows.forEach((r,i) => { lines.push([i+1,'"'+(r.name||'').replace(/"/g,'""')+'"','\t'+r.zazzleId,'"https://www.zazzle.com/store/minticdesignstudio/products?ps=128&dp='+r.zazzleId+'"',r.revenue.toFixed(2),r.units,r.customers,r.orders,r.highSignal?'Yes':'No',r.repeat?'Yes':'No',(r.highSignal&&r.repeat)?'Yes':'No'].join(',')); }); const blob = new Blob([lines.join('\n')],{type:'text/csv'}); const url = URL.createObjectURL(blob); const a = document.createElement('a'); a.href=url; a.download='best_sellers_'+tab+'.csv'; a.click(); URL.revokeObjectURL(url); if(showToast) showToast('CSV exportado'); }} style={{ padding: '7px 14px', background: theme.accent, color: '#fff', borderRadius: 6, fontSize: 12, fontWeight: 600, cursor: 'pointer', whiteSpace: 'nowrap' }}>Export CSV</div></div>
      </div>
        ...
        <div>
          ...
          {tab !== 'signals' && (
            <div style={{ background: theme.surface, border: '1px solid ' + theme.border, borderRadius: 8, padding: 20 }}>
              ...
              <div style={{ overflowX: 'auto' }}>
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
                  <thead><tr>{['#', 'Producto', 'Product ID', cfg.mainLabel, cfg.secLabel, 'Clientes', 'Senales'].map((h) => (<th key={h} style={{ textAlign: 'left', padding: '10px 12px', fontSize: 11, fontWeight: 600, color: theme.muted, textTransform: 'uppercase', borderBottom: '1px solid ' + theme.border, whiteSpace: 'nowrap' }}>{h}</th>))}</tr></thead>
                  <tbody>
                    {tableRows.map((r, i) => (
                      <tr key={r.stripped}>
                        <td style={{ padding: '10px 12px', borderBottom: '1px solid ' + theme.border, color: theme.muted, fontWeight: 700, fontSize: 12 }}>{i + 1}</td>
                        <td style={{ padding: '10px 12px', borderBottom: '1px solid ' + theme.border, maxWidth: 220, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', fontWeight: 500, color: theme.text }}><a href={'https://www.zazzle.com/store/minticdesignstudio/products?ps=128&dp=' + r.zazzleId} target="_blank" rel="noreferrer" style={{ color: theme.text, textDecoration: 'none' }} title={r.name}>{r.name}</a></td><td style={{ padding: '10px 12px', borderBottom: '1px solid ' + theme.border, color: theme.muted, fontSize: 11, fontFamily: 'monospace' }}><a href={'https://www.zazzle.com/store/minticdesignstudio/products?ps=128&dp=' + r.zazzleId} target="_blank" rel="noreferrer" style={{ color: theme.muted, textDecoration: 'none' }}>{r.zazzleId}</a></td>
                        <td style={{ padding: '10px 12px', borderBottom: '1px solid ' + theme.border, fontWeight: 700, color: theme.accent }}>{cfg.mainFmt(r[cfg.mainKey])}</td><td style={{ padding: '10px 12px', borderBottom: '1px solid ' + theme.border, color: theme.muted }}>{cfg.secFmt(r[cfg.secKey])}</td>
