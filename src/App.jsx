...
const theme = {
  ...
  accent: "#6C63FF", accentLight: "#8B84FF", low: "#4CAF50", medium: "#FF9800",
  high: "#E91E63", text: "#F0F0F5", muted: "#7A7D9C", success: "#4CAF50", danger: "#F44336",
};
...
function Toast({ msg, type, onClose }) {
  ...
  return (
    <div style={{
      ...
      background:theme.card, border:`1px solid ${type==="success"?theme.success:theme.danger}`,
...
function Dashboard({ products, actions, productTypes, niches }) {
  ...
  return (
    <div>
      <div style={{marginBottom:24}}><div style={{fontSize:22,fontWeight:700}}>Dashboard</div><div style={{fontSize:13,color:theme.muted,marginTop:4}}>Vista general de tu inventario</div></div>
      <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:16,marginBottom:20}}>
        {[["Hoy",count(todayStr)],["Esta semana",count(weekAgo)],["Este mes",count(monthStart)],["Este año",count(yearStart)]].map(([l,v])=>(
          <div key={l} style={{background:theme.card,border:`1px solid ${theme.border}`,borderRadius:10,padding:20}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16,marginBottom:20}}>
        <div style={{background:theme.card,border:`1px solid ${theme.border}`,borderRadius:10,padding:20}}>
          ...
          <div style={{fontSize:36,fontWeight:800,marginBottom:16}}>{products.length}</div>
          ...
          <div style={{display:"flex",gap:16,fontSize:12}}>
            {["low","medium","high"].map(k=>(
              <div key={k} style={{display:"flex",alignItems:"center",gap:4}}>
                ...
                <span style={{fontWeight:600}}>{lmhCounts[k]||0}</span>
              </div>
            ))}
          </div>
        </div>
        <div style={{background:theme.card,border:`1px solid ${theme.border}`,borderRadius:10,padding:20}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:12}}>
            {[["Hoy",actCount(todayStr)],["Semana",actCount(weekAgo)],["Mes",actCount(monthStart)]].map(([l,v])=>(
              <div key={l}><div style={{fontSize:24,fontWeight:800,color:theme.accentLight}}>{v}</div><div style={{fontSize:11,color:theme.muted}}>{l}</div></div>
            ))}
          </div>
        </div>
      </div>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16}}>
        <div style={{background:theme.card,border:`1px solid ${theme.border}`,borderRadius:10,padding:20}}>
          <div style={{fontSize:11,fontWeight:600,color:theme.muted,textTransform:"uppercase",marginBottom:12}}>Producción mensual</div>
        ...
        <div style={{background:theme.card,border:`1px solid ${theme.border}`,borderRadius:10,padding:20}}>
          ...
          {topNiches.map(([name,count])=>(
            <div key={name} style={{marginBottom:8}}>
              <div style={{display:"flex",justifyContent:"space-between",fontSize:12,marginBottom:3}}><span>{name}</span><span style={{color:theme.muted}}>{count}</span></div>
              <div style={{height:6,background:theme.border,borderRadius:3}}><div style={{height:"100%",background:theme.accent,borderRadius:3,width:`${count/topNiches[0][1]*100}%`}}/></div>
            </div>
          ))}
        </div>
      </div>
...
function QuickAdd({ productTypes, niches, onSave, showToast }) {
  ...
  return (
    <div>
      <div style={{marginBottom:24}}><div style={{fontSize:22,fontWeight:700}}>Quick Add</div><div style={{fontSize:13,color:theme.muted,marginTop:4}}>Pegá el texto de Zazzle y extrae los datos automáticamente</div></div>
      <div style={s}>
        <div style={{fontSize:11,fontWeight:600,color:theme.muted,textTransform:"uppercase",marginBottom:12}}>Paso 1 — Pegá el texto de Zazzle</div>
        ...
        <button onClick={handleParse} disabled={!text.trim()} style={{padding:"8px 16px",background:theme.accent,color:"#fff",border:"none",borderRadius:7,fontSize:13,fontWeight:600,cursor:"pointer"}}>⚡ Extraer datos</button>
        {parsed && (
          <div style={{background:theme.surface,border:`1px solid ${theme.border}`,borderRadius:8,padding:16,marginTop:12}}>
            <div style={{fontSize:12,fontWeight:600,color:theme.accentLight,marginBottom:10}}>Datos extraídos:</div>
            {[["Product ID",parsed.product_id],["Nombre",parsed.name],["Fecha",parsed.date],["Hora",parsed.time]].map(([k,v])=>(
              <div key={k} style={{display:"flex",gap:8,marginBottom:6,fontSize:13}}>
                ...
                <span style={{fontWeight:500}}>{v||<span style={{color:theme.danger}}>No detectado</span>}</span>
              </div>
            ))}
          </div>
        )}
      ...
      {parsed && (
        <div style={s}>
          <div style={{fontSize:11,fontWeight:600,color:theme.muted,textTransform:"uppercase",marginBottom:12}}>Paso 2 — Clasificación</div>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:12}}>
            <div><label style={lbl}>Product Type</label><select style={inp} value={selectedType} onChange={e=>setSelectedType(e.target.value)}><option value="">Seleccioná...</option>{productTypes.filter(t=>t.active).map(t=><option key={t.id} value={t.id}>{t.name} ({t.lmh?.toUpperCase()})</option>)}</select></div>
...
function ProductMaster({ products, productTypes, niches, onRefresh, showToast }) {
  ...
  return (
    <div>
      <div style={{marginBottom:24}}><div style={{fontSize:22,fontWeight:700}}>Product Master</div><div style={{fontSize:13,color:theme.muted,marginTop:4}}>{products.length} productos registrados</div></div>
      <div style={{background:theme.card,border:`1px solid ${theme.border}`,borderRadius:10,padding:20}}>
        ...
        <div style={{overflowX:"auto"}}>
          <table style={{width:"100%",borderCollapse:"collapse",fontSize:13}}>
            ...
            <tbody>
              {filtered.slice(0,100).map(p=>{
                ...
                return (
                  <tr key={p.id}>
                    <td style={{padding:"10px 12px",borderBottom:`1px solid ${theme.border}`,maxWidth:220,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",fontWeight:500}}>{p.name}</td>
                    <td style={{padding:"10px 12px",borderBottom:`1px solid ${theme.border}`,color:theme.muted,fontSize:11}}>{p.product_id}</td>
                    <td style={{padding:"10px 12px",borderBottom:`1px solid ${theme.border}`}}>{pt?.name||"—"}</td>
          ...
          {filtered.length>100&&<div style={{textAlign:"center",padding:12,color:theme.muted,fontSize:12}}>Mostrando 100 de {filtered.length} — usá los filtros</div>}
        </div>
      </div>
      {editProduct&&(
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.6)",zIndex:500,display:"flex",alignItems:"center",justifyContent:"center",padding:20}} onClick={e=>e.target===e.currentTarget&&setEditProduct(null)}>
          <div style={{background:theme.surface,border:`1px solid ${theme.border}`,borderRadius:14,padding:28,width:"100%",maxWidth:560,maxHeight:"90vh",overflowY:"auto"}}>
            <div style={{fontSize:17,fontWeight:700,marginBottom:20}}>Editar producto</div>
  ...
  return (
    <div>
      <div style={{marginBottom:24,display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
        <div><div style={{fontSize:22,fontWeight:700}}>Product Types</div><div style={{fontSize:13,color:theme.muted,marginTop:4}}>{productTypes.filter(t=>t.active).length} activos</div></div>
        <button onClick={()=>setShowAdd(true)} style={{padding:"8px 16px",background:theme.accent,color:"#fff",border:"none",borderRadius:7,fontSize:13,fontWeight:600,cursor:"pointer"}}>+ Agregar tipo</button>
      </div>
      <div style={{background:theme.card,border:`1px solid ${theme.border}`,borderRadius:10,padding:20}}>
        <input style={{...inp,marginBottom:16}} placeholder="Buscar..." value={search} onChange={e=>setSearch(e.target.value)}/>
        <div style={{overflowX:"auto"}}>
          <table style={{width:"100%",borderCollapse:"collapse",fontSize:13}}>
            <thead><tr>{["Nombre","Código Zazzle","LMH","Estado",""].map(h=><th key={h} style={{textAlign:"left",padding:"10px 12px",fontSize:11,fontWeight:600,color:theme.muted,textTransform:"uppercase",borderBottom:`1px solid ${theme.border}`}}>{h}</th>)}</tr></thead>
            <tbody>
              {filtered.map(t=>(
                <tr key={t.id} style={{opacity:t.active?1:0.4}}>
                  <td style={{padding:"10px 12px",borderBottom:`1px solid ${theme.border}`,fontWeight:500}}>{t.name}</td>
                  <td style={{padding:"10px 12px",borderBottom:`1px solid ${theme.border}`,color:theme.muted,fontSize:11}}>{t.zazzle_code||"—"}</td>
                  <td style={{padding:"10px 12px",borderBottom:`1px solid ${theme.border}`}}><span style={{padding:"2px 8px",borderRadius:20,fontSize:11,fontWeight:700,background:t.lmh==="low"?"rgba(76,175,80,0.15)":t.lmh==="medium"?"rgba(255,152,0,0.15)":"rgba(233,30,99,0.15)",color:lmhColor(t.lmh)}}>{t.lmh}</span></td>
                  ...
                  <td style={{padding:"10px 12px",borderBottom:`1px solid ${theme.border}`}}><button onClick={()=>toggleActive(t)} style={{padding:"3px 8px",background:theme.surface,border:`1px solid ${theme.border}`,borderRadius:5,color:theme.text,fontSize:11,cursor:"pointer"}}>{t.active?"Desactivar":"Activar"}</button></td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      {showAdd&&(
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.6)",zIndex:500,display:"flex",alignItems:"center",justifyContent:"center",padding:20}} onClick={e=>e.target===e.currentTarget&&setShowAdd(false)}>
          <div style={{background:theme.surface,border:`1px solid ${theme.border}`,borderRadius:14,padding:28,width:"100%",maxWidth:480}}>
            <div style={{fontSize:17,fontWeight:700,marginBottom:20}}>Nuevo Product Type</div>
            ...
            <div style={{marginBottom:12}}><label style={{fontSize:11,fontWeight:600,color:theme.muted,textTransform:"uppercase",display:"block",marginBottom:5}}>Código Zazzle (opcional)</label><input style={inp} value={form.zazzle_code} onChange={e=>setForm({...form,zazzle_code:e.target.value})} placeholder="Ej: zazzle_flatappointmentcard"/></div>
            ...
            <div style={{display:"flex",gap:10,justifyContent:"flex-end"}}>
              <button onClick={()=>setShowAdd(false)} style={{padding:"8px 16px",background:theme.card,border:`1px solid ${theme.border}`,borderRadius:7,color:theme.text,fontSize:13,cursor:"pointer"}}>Cancelar</button>
              <button onClick={handleAdd} disabled={saving} style={{padding:"8px 16px",background:theme.accent,border:"none",borderRadius:7,color:"#fff",fontSize:13,fontWeight:600,cursor:"pointer"}}>{saving?"Guardando...":"Agregar"}</button>
            </div>
          </div>
        </div>
      )}
...
function Niches({ niches, nicheCategories, onRefresh, showToast }) {
  ...
  const inp = { width:"100%", padding:"9px 12px", background:theme.surface, border:`1px solid ${theme.border}`, borderRadius:7, color:theme.text, fontSize:13, outline:"none" };
  const handleAdd = async () => {
    if (!form.name||!form.category_id) return showToast("Completá todos los campos","error");
    setSaving(true);
  ...
  return (
    <div>
      <div style={{marginBottom:24,display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
        <div><div style={{fontSize:22,fontWeight:700}}>Niches</div><div style={{fontSize:13,color:theme.muted,marginTop:4}}>{niches.filter(n=>n.active).length} activos</div></div>
        <button onClick={()=>setShowAdd(true)} style={{padding:"8px 16px",background:theme.accent,color:"#fff",border:"none",borderRadius:7,fontSize:13,fontWeight:600,cursor:"pointer"}}>+ Agregar niche</button>
      </div>
      {nicheCategories.map(cat=>{
        const catNiches=niches.filter(n=>n.category_id===cat.id);
        return (
          <div key={cat.id} style={{background:theme.card,border:`1px solid ${theme.border}`,borderRadius:10,padding:20,marginBottom:16}}>
              ...
              <tbody>
                {catNiches.map(n=>(
                  <tr key={n.id} style={{opacity:n.active?1:0.4}}>
                    <td style={{padding:"8px 12px",borderBottom:`1px solid ${theme.border}`,fontWeight:500}}>{n.name}</td>
                    ...
                    <td style={{padding:"8px 12px",borderBottom:`1px solid ${theme.border}`}}><button onClick={()=>toggleActive(n)} style={{padding:"3px 8px",background:theme.surface,border:`1px solid ${theme.border}`,borderRadius:5,color:theme.text,fontSize:11,cursor:"pointer"}}>{n.active?"Desactivar":"Activar"}</button></td>
                  </tr>
                ))}
      ...
      {showAdd&&(
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.6)",zIndex:500,display:"flex",alignItems:"center",justifyContent:"center",padding:20}} onClick={e=>e.target===e.currentTarget&&setShowAdd(false)}>
          <div style={{background:theme.surface,border:`1px solid ${theme.border}`,borderRadius:14,padding:28,width:"100%",maxWidth:480}}>
            <div style={{fontSize:17,fontWeight:700,marginBottom:20}}>Nuevo Niche</div>
            <div style={{marginBottom:12}}><label style={{fontSize:11,fontWeight:600,color:theme.muted,textTransform:"uppercase",display:"block",marginBottom:5}}>Categoría</label><select style={inp} value={form.category_id} onChange={e=>setForm({...form,category_id:e.target.value})}><option value="">Seleccioná...</option>{nicheCategories.map(c=><option key={c.id} value={c.id}>{c.name}</option>)}</select></div>
...
function BestSellerActions({ actions, onRefresh, showToast }) {
  ...
  return (
    <div>
      <div style={{marginBottom:24,display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
        <div><div style={{fontSize:22,fontWeight:700}}>Best Seller Actions</div><div style={{fontSize:13,color:theme.muted,marginTop:4}}>{actions.length} acciones registradas</div></div>
        <div style={{display:"flex",gap:8}}>
          <button onClick={exportCSV} style={{padding:"8px 16px",background:theme.card,border:`1px solid ${theme.border}`,borderRadius:7,color:theme.text,fontSize:13,cursor:"pointer"}}>↓ CSV</button>
          <button onClick={()=>setShowAdd(true)} style={{padding:"8px 16px",background:theme.accent,color:"#fff",border:"none",borderRadius:7,fontSize:13,fontWeight:600,cursor:"pointer"}}>+ Nueva acción</button>
        </div>
      </div>
      <div style={{background:theme.card,border:`1px solid ${theme.border}`,borderRadius:10,padding:20}}>
        <input style={{...inp,marginBottom:16}} placeholder="Buscar..." value={search} onChange={e=>setSearch(e.target.value)}/>
        <div style={{overflowX:"auto"}}>
          <table style={{width:"100%",borderCollapse:"collapse",fontSize:13}}>
            <thead><tr>{["Fecha","Producto","Acción","Outcome","Próximo paso"].map(h=><th key={h} style={{textAlign:"left",padding:"10px 12px",fontSize:11,fontWeight:600,color:theme.muted,textTransform:"uppercase",borderBottom:`1px solid ${theme.border}`,whiteSpace:"nowrap"}}>{h}</th>)}</tr></thead>
            <tbody>
              {filtered.map(a=>(
                <tr key={a.id}>
                  <td style={{padding:"10px 12px",borderBottom:`1px solid ${theme.border}`,whiteSpace:"nowrap",color:theme.muted}}>{a.date}</td>
                  <td style={{padding:"10px 12px",borderBottom:`1px solid ${theme.border}`}}><div style={{fontWeight:500,maxWidth:180,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{a.product_name||a.product_id}</div><div style={{fontSize:11,color:theme.muted}}>{a.product_id}</div></td>
                  <td style={{padding:"10px 12px",borderBottom:`1px solid ${theme.border}`,maxWidth:200}}>{a.action}</td>
                  <td style={{padding:"10px 12px",borderBottom:`1px solid ${theme.border}`,maxWidth:160,color:theme.muted,fontSize:12}}>{a.outcome}</td>
                  <td style={{padding:"10px 12px",borderBottom:`1px solid ${theme.border}`,maxWidth:160,color:theme.accentLight,fontSize:12}}>{a.next_step}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      {showAdd&&(
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.6)",zIndex:500,display:"flex",alignItems:"center",justifyContent:"center",padding:20}} onClick={e=>e.target===e.currentTarget&&setShowAdd(false)}>
          <div style={{background:theme.surface,border:`1px solid ${theme.border}`,borderRadius:14,padding:28,width:"100%",maxWidth:560,maxHeight:"90vh",overflowY:"auto"}}>
            <div style={{fontSize:17,fontWeight:700,marginBottom:20}}>Nueva Best Seller Action</div>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:12}}>
            ...
            <div style={{marginBottom:12}}><label style={lbl}>Acción realizada</label><textarea style={{...inp,minHeight:60,resize:"vertical"}} value={form.action} onChange={e=>setForm({...form,action:e.target.value})}/></div>
            ...
            <div style={{marginBottom:12}}><label style={lbl}>Próximo paso</label><input style={inp} value={form.next_step} onChange={e=>setForm({...form,next_step:e.target.value})}/></div>
            ...
            <div style={{display:"flex",gap:10,justifyContent:"flex-end"}}>
              <button onClick={()=>setShowAdd(false)} style={{padding:"8px 16px",background:theme.card,border:`1px solid ${theme.border}`,borderRadius:7,color:theme.text,fontSize:13,cursor:"pointer"}}>Cancelar</button>
              <button onClick={handleAdd} disabled={saving} style={{padding:"8px 16px",background:theme.accent,border:"none",borderRadius:7,color:"#fff",fontSize:13,fontWeight:600,cursor:"pointer"}}>{saving?"Guardando...":"Guardar"}</button>
            </div>
          </div>
        </div>
      )}
...
function ImportCSV({ productTypes, niches, onRefresh, showToast }) {
  ...
  return (
    <div>
      <div style={{marginBottom:24}}><div style={{fontSize:22,fontWeight:700}}>Importar CSV</div><div style={{fontSize:13,color:theme.muted,marginTop:4}}>Cargá tu production log desde Google Sheets</div></div>
      <div style={{background:theme.card,border:`1px solid ${theme.border}`,borderRadius:10,padding:20,marginBottom:16}}>
        ...
        <div style={{background:theme.surface,borderRadius:6,padding:12,fontSize:12,color:theme.muted,fontFamily:"monospace",marginBottom:12}}>Product ID | Product Name | Product Type | Niche | Creation Date</div>
        <div style={{fontSize:12,color:theme.muted,lineHeight:1.8}}>
          • Separado por tabs (TSV) — exportá desde Google Sheets como TSV<br/>
          • Product Type debe coincidir con nombre existente o código Zazzle<br/>
          • Duplicados ignorados automáticamente por Product ID
        </div>
      </div>
      <div style={{background:theme.card,border:`1px solid ${theme.border}`,borderRadius:10,padding:20}}>
        <div style={{fontSize:11,fontWeight:600,color:theme.muted,textTransform:"uppercase",marginBottom:8}}>Seleccioná el archivo</div>
        <input type="file" accept=".csv,.tsv,.txt" onChange={handleFile} style={{color:theme.text,marginBottom:16,display:"block"}}/>
        {rows.length>0&&(
          <div style={{marginBottom:16}}>
            <div style={{fontSize:13,marginBottom:8}}><span style={{color:theme.accentLight,fontWeight:700}}>{rows.length}</span> filas detectadas</div>
                ...
                <tbody>
                  {rows.slice(0,10).map((r,i)=>(
                    <tr key={i}>
                      ...
                      <td style={{padding:"6px 10px",borderBottom:`1px solid ${theme.border}`,maxWidth:180,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{r.product_name}</td>
                      <td style={{padding:"6px 10px",borderBottom:`1px solid ${theme.border}`}}>{r.product_type}</td>
                      <td style={{padding:"6px 10px",borderBottom:`1px solid ${theme.border}`}}>{r.niche}</td>
            ...
            <button onClick={handleImport} disabled={importing} style={{padding:"8px 16px",background:theme.accent,color:"#fff",border:"none",borderRadius:7,fontSize:13,fontWeight:600,cursor:"pointer"}}>{importing?"Importando...":"⬆ Importar "+rows.length+" productos"}</button>
          </div>
        )}
        {results&&(
...
function Analytics({ products, productTypes, niches, nicheCategories }) {
  ...
  return (
    <div>
      <div style={{marginBottom:24}}><div style={{fontSize:22,fontWeight:700}}>Analytics</div><div style={{fontSize:13,color:theme.muted,marginTop:4}}>Análisis de distribución e inventario</div></div>
      <div style={{background:theme.card,border:`1px solid ${theme.border}`,borderRadius:10,padding:20,marginBottom:16}}>
        <div style={{fontSize:11,fontWeight:600,color:theme.muted,textTransform:"uppercase",marginBottom:12}}>Distribución LMH — {products.length} productos</div>
        <div style={{display:"flex",gap:24,marginBottom:16}}>
          {["low","medium","high"].map(k=>(
            <div key={k}><div style={{fontSize:28,fontWeight:800,color:lmhColor(k)}}>{((lmhCounts[k]||0)/total*100).toFixed(1)}%</div><div style={{fontSize:12,color:theme.muted,textTransform:"capitalize"}}>{k} — {lmhCounts[k]||0} productos</div></div>
          ))}
        </div>
        <div style={{display:"flex",height:12,borderRadius:4,overflow:"hidden",gap:2}}>
          {["low","medium","high"].map(k=><div key={k} style={{width:`${(lmhCounts[k]||0)/total*100}%`,background:lmhColor(k),borderRadius:2}}/>)}
        </div>
        {lmhCounts.low>lmhCounts.high*3&&<div style={{marginTop:12,padding:"8px 12px",background:"rgba(255,152,0,0.1)",border:"1px solid rgba(255,152,0,0.3)",borderRadius:6,fontSize:12,color:theme.medium}}>⚠ Tu inventario tiene mucho Low en relación a High. Considerá producir más High royalty.</div>}
      </div>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16,marginBottom:16}}>
        <div style={{background:theme.card,border:`1px solid ${theme.border}`,borderRadius:10,padding:20}}>
          <div style={{fontSize:11,fontWeight:600,color:theme.muted,textTransform:"uppercase",marginBottom:12}}>Por categoría</div>
          {sortedCats.map(([name,count])=>(
            <div key={name} style={{marginBottom:10}}>
              <div style={{display:"flex",justifyContent:"space-between",fontSize:13,marginBottom:3}}><span>{name}</span><span style={{color:theme.muted}}>{count} ({(count/total*100).toFixed(1)}%)</span></div>
              <div style={{height:6,background:theme.border,borderRadius:3}}><div style={{height:"100%",background:theme.accent,borderRadius:3,width:`${count/total*100}%`}}/></div>
            </div>
          ))}
        </div>
        <div style={{background:theme.card,border:`1px solid ${theme.border}`,borderRadius:10,padding:20}}>
          ...
          {sortedNiches.slice(0,10).map(([name,count])=>(
            <div key={name} style={{marginBottom:8}}>
              <div style={{display:"flex",justifyContent:"space-between",fontSize:12,marginBottom:2}}><span>{name}</span><span style={{color:theme.muted}}>{count}</span></div>
              <div style={{height:6,background:theme.border,borderRadius:3}}><div style={{height:"100%",background:theme.accent,borderRadius:3,width:`${count/sortedNiches[0][1]*100}%`}}/></div>
            </div>
          ))}
        </div>
      </div>
          ...
          <table style={{width:"100%",borderCollapse:"collapse",fontSize:13}}>
            ...
            <tbody>
              {sortedTypes.slice(0,20).map(([name,count])=>{
                ...
                return (
                  <tr key={name}>
                    <td style={{padding:"10px 12px",borderBottom:`1px solid ${theme.border}`,fontWeight:500}}>{name}</td>
                    <td style={{padding:"10px 12px",borderBottom:`1px solid ${theme.border}`}}>{pt&&<span style={{padding:"2px 8px",borderRadius:20,fontSize:11,fontWeight:700,background:pt.lmh==="low"?"rgba(76,175,80,0.15)":pt.lmh==="medium"?"rgba(255,152,0,0.15)":"rgba(233,30,99,0.15)",color:lmhColor(pt.lmh)}}>{pt.lmh}</span>}</td>
                    <td style={{padding:"10px 12px",borderBottom:`1px solid ${theme.border}`,fontWeight:700,color:theme.accentLight}}>{count}</td>
                    <td style={{padding:"10px 12px",borderBottom:`1px solid ${theme.border}`,color:theme.muted}}>{(count/total*100).toFixed(1)}%</td>
                    <td style={{padding:"10px 12px",borderBottom:`1px solid ${theme.border}`,width:120}}><div style={{height:6,background:theme.border,borderRadius:3}}><div style={{height:"100%",background:theme.accent,borderRadius:3,width:`${count/sortedTypes[0][1]*100}%`}}/></div></td>
                  </tr>
                );
              })}
            </tbody>
...
function Reports({ products, actions, productTypes, niches }) {
  ...
  const exportCSV = () => {
    const lines = [["MINTIC PRODUCTION HUB — REPORTE"],[`Período: ${period==="day"?"Hoy":period==="week"?"Esta semana":"Este mes"}`],[`Fecha: ${now.toLocaleDateString()}`],[],["PRODUCCIÓN"],["Total",periodProducts.length],["Low",lmhCount.low],["Medium",lmhCount.medium],["High",lmhCount.high],[],["NICHES"],...Object.entries(nicheCount).sort((a,b)=>b[1]-a[1]).map(([k,v])=>[k,v]),[],["PRODUCT TYPES"],...Object.entries(typeCount).sort((a,b)=>b[1]-a[1]).map(([k,v])=>[k,v]),[],["BEST SELLER ACTIONS",periodActions.length]];
  ...
  return (
    <div>
      <div style={{marginBottom:24,display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
        <div><div style={{fontSize:22,fontWeight:700}}>Reportes</div><div style={{fontSize:13,color:theme.muted,marginTop:4}}>Resumen de producción y acciones</div></div>
        <button onClick={exportCSV} style={{padding:"8px 16px",background:theme.card,border:`1px solid ${theme.border}`,borderRadius:7,color:theme.text,fontSize:13,cursor:"pointer"}}>↓ Exportar CSV</button>
      </div>
      <div style={{display:"flex",gap:4,marginBottom:20,background:theme.surface,padding:4,borderRadius:8,width:"fit-content"}}>
        {["day","week","month"].map(p=><div key={p} onClick={()=>setPeriod(p)} style={{padding:"6px 14px",borderRadius:6,fontSize:13,fontWeight:500,cursor:"pointer",background:period===p?theme.accent:"transparent",color:period===p?"#fff":theme.muted}}>{label[p]}</div>)}
      </div>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:16,marginBottom:16}}>
        <div style={{background:theme.card,border:`1px solid ${theme.border}`,borderRadius:10,padding:20}}><div style={{fontSize:11,fontWeight:600,color:theme.muted,textTransform:"uppercase",marginBottom:8}}>Productos</div><div style={{fontSize:36,fontWeight:800}}>{periodProducts.length}</div></div>
        <div style={{background:theme.card,border:`1px solid ${theme.border}`,borderRadius:10,padding:20}}><div style={{fontSize:11,fontWeight:600,color:theme.muted,textTransform:"uppercase",marginBottom:8}}>BS Actions</div><div style={{fontSize:36,fontWeight:800}}>{periodActions.length}</div></div>
        <div style={{background:theme.card,border:`1px solid ${theme.border}`,borderRadius:10,padding:20}}><div style={{fontSize:11,fontWeight:600,color:theme.muted,textTransform:"uppercase",marginBottom:8}}>LMH</div><div style={{display:"flex",gap:12,marginTop:4}}>{["low","medium","high"].map(k=><div key={k}><div style={{fontSize:20,fontWeight:800,color:lmhColor(k)}}>{lmhCount[k]||0}</div><div style={{fontSize:10,color:theme.muted,textTransform:"capitalize"}}>{k}</div></div>)}</div></div>
      </div>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16}}>
        <div style={{background:theme.card,border:`1px solid ${theme.border}`,borderRadius:10,padding:20}}>
          <div style={{fontSize:11,fontWeight:600,color:theme.muted,textTransform:"uppercase",marginBottom:12}}>Niches</div>
          {Object.entries(nicheCount).sort((a,b)=>b[1]-a[1]).map(([k,v])=><div key={k} style={{display:"flex",justifyContent:"space-between",padding:"6px 0",borderBottom:`1px solid ${theme.border}`,fontSize:13}}><span>{k}</span><span style={{color:theme.accentLight,fontWeight:700}}>{v}</span></div>)}
          {Object.keys(nicheCount).length===0&&<div style={{color:theme.muted,fontSize:12}}>Sin datos en este período</div>}
        </div>
        <div style={{background:theme.card,border:`1px solid ${theme.border}`,borderRadius:10,padding:20}}>
          <div style={{fontSize:11,fontWeight:600,color:theme.muted,textTransform:"uppercase",marginBottom:12}}>Product Types</div>
          {Object.entries(typeCount).sort((a,b)=>b[1]-a[1]).map(([k,v])=><div key={k} style={{display:"flex",justifyContent:"space-between",padding:"6px 0",borderBottom:`1px solid ${theme.border}`,fontSize:13}}><span>{k}</span><span style={{color:theme.accentLight,fontWeight:700}}>{v}</span></div>)}
          {Object.keys(typeCount).length===0&&<div style={{color:theme.muted,fontSize:12}}>Sin datos en este período</div>}
        </div>
      </div>
    </div>
  );
...
const NAV = [
  ...
  { id:"types", label:"Product Types", icon:"🏷" },
    ...
    <div style={{display:"flex",minHeight:"100vh",background:theme.bg,color:theme.text,fontFamily:"Inter, system-ui, sans-serif"}}>
      <aside style={{width:220,minWidth:220,background:theme.surface,borderRight:`1px solid ${theme.border}`,padding:"24px 0",display:"flex",flexDirection:"column",position:"sticky",top:0,height:"100vh"}}>
        <div style={{padding:"0 20px 24px",borderBottom:`1px solid ${theme.border}`}}>
          <div style={{fontSize:13,fontWeight:700,color:theme.accentLight,letterSpacing:"0.08em",textTransform:"uppercase"}}>Mintic Hub</div>
        ...
        <div style={{padding:"16px 20px",borderTop:`1px solid ${theme.border}`,fontSize:11,color:theme.muted}}>{products.length} productos · V1</div>
      </aside>
