...
async function fetchSuccessRate(start, end) {
  ...
  const BASELINE_TOTAL = 3741;
  const { count: newCount } = await supabase.from("products").select("id",{count:"exact",head:true}).gt("created_date", BASELINE_DATE);
  const totalProducts = BASELINE_TOTAL + (newCount||0);
  return { sellingProducts: selling, totalProducts, successRate: totalProducts > 0 ? (selling/totalProducts)*100 : 0 };
}
